...
function setup() {
	...
	paperBody=Bodies.circle(50,350,10,options);
...
function draw() {
  ...
  rotate(paperBody.angle);
  pop();
  fill("violet");
  paper=ellipse(paperBody.position.x,paperBody.position.y,20,20);
  //paper.x=paper.x+2;
  keyPressed();
  drawSprites();
...
function keyPressed(){
	if(keyCode===UP_ARROW){
		...
		Matter.Body.applyForce(paperBody,paperBody.position,{x:10,y:-5});
	}	
}
